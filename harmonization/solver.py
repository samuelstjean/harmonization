...
def solve_l1(X, D, alpha=None, return_all=False, nlambdas=100, ncores=-1, positivity=False, variance=None, fit_intercept=True, standardize=True,
    ...
    # D = np.array(D, copy=True, dtype=np.float64, order='F')
    ...
    if use_joblib:
        stuff = Parallel(n_jobs=ncores, mmap_mode=None, backend='threading')(delayed(lasso_path_parallel)(*args) for args in arglist)
    else:
